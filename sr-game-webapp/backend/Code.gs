/**
 * 社労士ゲームアプリ - Google Apps Script バックエンド
 * スプレッドシートと統合してデータ管理
 */
...
function doPost(e) {
  try {
    ...
    if (data.action === 'registerUser') {
      const result = registerUser(data.name, data.email);
      return ContentService
        .createTextOutput(JSON.stringify({ success: true, user: result }))
        .setMimeType(ContentService.MimeType.JSON);
    }

    // プレイデータ保存
    if (data.action === 'savePlayData') {
      const result = savePlayData(data);
      return ContentService
